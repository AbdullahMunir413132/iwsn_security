    ...
    while ((res = pcap_next_ex(handle, &header, &packet)) >= 0) {
        ...
        if (parse_packet(dpi_engine, packet, header->caplen, header->ts, &parsed) == 0) {
            ...
            if (packet_count % 1000 == 0) {
                printf("  ... processed %u packets, %u flows, %lu attacks detected\r", 
...
int main(int argc, char *argv[]) {
    ...
    // Analyze all flows for attack patterns
    ...
    // 3. MQTT Protocol Statistics (NEW!)
    print_mqtt_statistics();
    
    // 4. Detailed Attack Analysis (only if attacks detected)
    ...
    // 5. Show sample flows (first 5 flows with all packets)
    printf("\n\n");
    printf("╔════════════════════════════════════════════════════════════════╗\n");
    printf("║                                                                ║\n");
    printf("║               SAMPLE FLOW ANALYSIS (First 5 Flows)            ║\n");
    printf("║                                                                ║\n");
    printf("╚════════════════════════════════════════════════════════════════╝\n");
    
    uint32_t flows_to_show = (dpi_engine->flow_count < 5) ? dpi_engine->flow_count : 5;
    for (uint32_t i = 0; i < flows_to_show; i++) {
